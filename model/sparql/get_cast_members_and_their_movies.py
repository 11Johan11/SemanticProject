...
#TODO: Perhaps add the option to only pick N cast_members (to only prioritize famous actors and lower query count?)
def get_cast_members_and_their_movies(movie_ids):

    movie_filter = " ".join(f"wd:{movie}" for movie in movie_ids) 

    query = """
    SELECT ?otherMovie ?otherMovieName ?castMember ?castMemberName WHERE {{
...
#get_cast_members_and_their_movies(["Q153723","Q14786561"]) #Inglorious Bastards "Q153723" and Fury "Q14786561"